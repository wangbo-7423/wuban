...
- 长期记忆（上下文工程）：MCP 知识图谱按「L0 常驻层（偏好/目标/误区/兴趣）+ L1 检索层（当前消息按实体名+别名双通道命中的已学概念 + 一步邻接关系）」分层注入，聊天后异步抽取实体/别名/关系回写图谱（含「自发关联 / 表里反差探针 / 兴趣」三个学习信号约定，见 docs/00 §5.1）；
...
- **交互可视化**：`make_visual` 工具生成可拖拽 HTML 实验卡——`harmonic`/`pid_tuner` 概念模板 +
  `curve_lab` 数据驱动家族（LLM 只给数学表达式，sympy 白名单编译，sympy→JS 零 eval），
  沙箱 iframe 渲染 + 「先预测、再动手、后汇报」探究式引导（docs/16）；
- 质量保障：后端 pytest 单测 247 个（压缩/意图路由/卡片解析/会话记忆/流式编排/选项反馈/复习调度/上下文折算等纯逻辑），前端 vue-tsc 类型检查。
...
| 03 | [卡片协议](./docs/03-卡片协议.md) | `CardMessage` 全字段定义、17 种卡片类型与 payload 结构 |
